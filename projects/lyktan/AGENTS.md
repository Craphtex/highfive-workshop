...
3. Leverera med PR från branchen `team/lyktan` mot `main`. Skriv i PR-texten vad ni bidrar med och hur det syns.
4. Rör inte andra teams mappar eller filer. Vill ni ändra något gemensamt: PR och en rad i `#bygge`.
5. Ni får bygga **både frontend och backend**. Backend: `board/plugins/lyktan/index.js` monteras på `/t/lyktan/` och får ett API mot Torget (`board.post`, `board.query`, `onMessage`), se `board/plugins/README.md`. Frontend: `board/public/staden/kvarter/lyktan/index.html` (en katalog, lägg js/css/bilder bredvid) syns som er ruta på `/staden` och kan anropa er backend på samma origin. Båda dyker upp när PR:en mergats och deployats.
